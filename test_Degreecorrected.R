#Degree corrected model with estimated K or true K for G_{ij}
set.seed(3000)
library(Matrix)
...
sa=function(n,X)#Estimate K
{
  ...
  d=Spi$values
  a=colSums(X)
  cri=sqrt(2.01)*sqrt(max(a)*log(n))
  sum(abs(d)>cri)
  
}
samp=function(n,k0,PM,i1,j1,j2)#Calculate G_{ij}, K0 is the true K, n is dimension of the matrix, PM is the expectation of the adjacency matrix, we test the equality of i1, j1 and i1,j2
{
  el=eigs_sym(PM,5)
  e3=el$vectors[,1]
  e4=el$vectors[,2:5]
  ...
  #   k=max(sa(n,X),2)#Estimated K
      k=k0# True K
  thr=qchisq(0.95, df=k-1)
  ...
  for (a in 2:k)
    for (b  in 2:k)
    {
      va1=d[1]/d[a]/e3[i1,1]*e3[,a]-e3[i1,a]/e3[i1,1]/e3[i1,1]*e3[,1]
      vb1=d[1]/d[b]/e3[i1,1]*e3[,b]-e3[i1,b]/e3[i1,1]/e3[i1,1]*e3[,1]
      va2=d[1]/d[a]/e3[j2,1]*e3[,a]-e3[j2,a]/e3[j2,1]/e3[j2,1]*e3[,1]
      vb2=d[1]/d[b]/e3[j2,1]*e3[,b]-e3[j2,b]/e3[j2,1]/e3[j2,1]*e3[,1]
      cov1[a-1,b-1]=sum(W2[i1,]*va1*vb1+W2[j2,]*va2*vb2)-W2[i1,j2]*(va1[j2]*vb1[j2]+va2[i1]*vb2[i1])+W2[i1,j2]*(va1[j2]-va2[i1])*(vb1[j2]-vb2[i1])
    }
  cov=cov1/d[1]/d[1]
  
  st=e3[i1,2:k]/e1[i1]-e3[j2,2:k]/e1[j2]
 f[2,j]=t(st)%*%solve(cov)%*%st  }
  f
}
samk=function(n,k0,PM,i1,j1)#This code is to calculate the estimated K with o repetition
{
  f=rep(0,o)
  for (j in 1:o)
  {X <- sample_sbm(PM,n,k0)
...
x=0.2;rho=0.2;n0=n/5;n1=(n-3*n0)/4#x=0.2 is equal to the first entry of membership vector a_1
r=0.5/log(n)#r represents r^2 in the paper
le=length(r)
...
thr=qchisq(0.95, df=k0-1)
for (i in 1:le)
{
  mee[1,i]=sum(mea[i,]>thr)/o
  mee[2,i]=sum(mea[i+le,]>thr)/o
  mee[3,i]=mean(mea[i,])
  mee[4,i]=var(mea[i,]>thr)
}
mee#the first two rows of mee are the p-values of G_{i1j1} and G_{i2j2} 
